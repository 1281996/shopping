package com.mallika.date;

import com.mallika.date.MyDate;

public class DatDifferenceProvider {
		...
		public  int calDateDiff(MyDate startDate, MyDate endDate) {
			if (sameDate(startDate, endDate) && sameMonth(startDate, endDate) && sameYear(startDate, endDate)) {
				...
				return remainingDaysOfMonth(startDate) + daysInIntervingMonth(startDate, endDate) + endDate.getDd()+leaMonth(startDate,endDate);
			} else {
				return remainingDaysOfMonth(startDate) + remainingDaysInYear(startDate) + leadingYear(endDate)
						+ daysInInterveningYear(startDate, endDate)+leaYear(startDate,endDate);
			}
		...
		private int leaYear(MyDate startDate,MyDate endDate ){
			         int totalDays=0;
     ...
     private int leaMonth(MyDate startDate,MyDate endDate){
    	    int totalDaysInMonth=0;
    	    
    	    if((startDate.getYyyy()%400) ==0 ||  startDate.getYyyy()%4==0 &&  startDate.getYyyy()%100!=0){
    	    	 for(int index= startDate.getMm()-1;index<endDate.getMm();index++){
    	    	if(months[index]==FEB){
